...
;; Maximum number of identifiers that can be highlighted as type name
...
(defun go--find-enclosing-parentheses (position)
  "Return points of toplevel '(' and ')' surrounding POSITION if
such parentheses exist.

If toplevel '(' exists but ')' does not, it returns next blank
line or end-of-buffer position instead of position of closing
parenthesis.

If starting parenthesis is not found, it returns (POSITION
POSITION).
"
  (save-excursion
    (let ((beg)
...
(defun go--font-lock-extend-after-change-region-function (beg end old-len)
  "Determine the region to be scanned and fontified by Font Lock mode.

The scan normally starts at the beginning of a line, and in most
cases that works pretty well. An exception
is (). `go--match-func' needs to scan a function parameter list
from an occurrence of keyword \"func\" to end of its parameter
list. If there is a newline in a parameter list, Font Lock mode
let the function to start scanning from a middle of the parameter
list, which result in a wrong fontification.

This function extends the region to be scanned to include a whole
expression if the current line is parenthesized."
  (save-excursion
    (save-match-data
      ;; If we are at the beginning of a line, include the end of previous line,
      ;; because it's likely that we just modified the previous line -- a
      ;; newline was inserted at the end of it. Note that Font Lock mode usually
      ;; gives a line beginning position. We call line-beginning-position to
      ;; make sure it's always satisfied.
      (setq beg (if (bolp)
                    (min beg (line-beginning-position 0))
                  (line-beginning-position)))
      ;; So is EOL.
      (setq end (if (eolp)
                    (max end (line-end-position 0))
                  (line-end-position)))
      (multiple-value-bind (beg2 end2)
          (go--find-enclosing-parentheses beg)
        (if (> end2 end)
            (cons beg2 end2)
          (multiple-value-bind (beg3 end3)
              (go--find-enclosing-parentheses end)
            (cons (min beg beg2 beg3)
                  (max end end2 end3))))))))

(defun go--search-next-comma (end)
  "Search forward from point for a comma whose nesting level is
the same as point. If it reaches end of line or a closing
...
parameter list, and sets the identifier positions as the results
...
\(https://github.com/dominikh/yasnippet-go)"
  ...
  (set (make-local-variable 'font-lock-extend-after-change-region-function)
       #'go--font-lock-extend-after-change-region-function)
